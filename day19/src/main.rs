...
fn next_states(state: &State, blueprint: &Blueprint) -> Vec<State> {
    ...
    if state.inventory.ore >= blueprint.geode_robot_costs_ore
        && state.inventory.obsidian >= blueprint.geode_robot_costs_obsidian
    {
        next_states.push(State {
            depth: state.depth + 1,
            inventory: Inventory {
                ore: state.inventory.ore + state.inventory.ore_robots
                    - blueprint.geode_robot_costs_ore,
                clay: state.inventory.clay + state.inventory.clay_robots,
                obsidian: state.inventory.obsidian + state.inventory.obsidian_robots
                    - blueprint.geode_robot_costs_obsidian,
        ...
        if state.inventory.ore >= blueprint.obsidian_robot_costs_ore
            && state.inventory.clay >= blueprint.obsidian_robot_costs_clay
            && state.inventory.obsidian_robots < blueprint.geode_robot_costs_obsidian
        {
            next_states.push(State {
                depth: state.depth + 1,
                inventory: Inventory {
                    ore: state.inventory.ore + state.inventory.ore_robots
                        - blueprint.obsidian_robot_costs_ore,
                    clay: state.inventory.clay + state.inventory.clay_robots
                        - blueprint.obsidian_robot_costs_clay,
                    obsidian: state.inventory.obsidian + state.inventory.obsidian_robots,
                    geodes: state.inventory.geodes + state.inventory.geode_robots,
                    ore_robots: state.inventory.ore_robots,
                    clay_robots: state.inventory.clay_robots,
        ...
        if state.inventory.ore >= blueprint.clay_robot_costs_ore
            && state.inventory.clay_robots < blueprint.obsidian_robot_costs_clay
        {
            next_states.push(State {
                depth: state.depth + 1,
                inventory: Inventory {
                    ore: state.inventory.ore + state.inventory.ore_robots
                        - blueprint.clay_robot_costs_ore,
                    clay: state.inventory.clay + state.inventory.clay_robots,
                    obsidian: state.inventory.obsidian + state.inventory.obsidian_robots,
                    geodes: state.inventory.geodes + state.inventory.geode_robots,
                    ore_robots: state.inventory.ore_robots,
        ...
        if state.inventory.ore >= blueprint.ore_robot_costs_ore
            && state.inventory.ore_robots < blueprint.max_costs_ore
        {
            next_states.push(State {
                depth: state.depth + 1,
                inventory: Inventory {
                    ore: state.inventory.ore + state.inventory.ore_robots
                        - blueprint.ore_robot_costs_ore,
                    clay: state.inventory.clay + state.inventory.clay_robots,
                    obsidian: state.inventory.obsidian + state.inventory.obsidian_robots,
                    geodes: state.inventory.geodes + state.inventory.geode_robots,
...
fn part2(input: &str) -> usize {
    ...
    println!("{}", max_geodes_1);

    let max_geodes_2 = dfs(
        &State {
            depth: 0,
            inventory: Inventory {
                ore: 0,
    ...
    println!("{}", max_geodes_2);

    let max_geodes_3 = dfs(
        &State {
            depth: 0,
            inventory: Inventory {
                ore: 0,
    ...
    println!("{}", max_geodes_3);

    max_geodes_1 as usize * max_geodes_2 as usize * max_geodes_3 as usize
}
